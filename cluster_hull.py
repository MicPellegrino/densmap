...
# Parameters
Ly = 4.67650
xmin = 50.834055193939996
xmax = 53.674944806060005
# xmin = 46.00522627041621
# xmax = 51.2321737295838
...
output_file_name = "contact_line.mp4"
...
with writer.saving(fig, output_file_name, 250):
    ...
    for frame in range(frame_init, frame_fin+1) :
        ...
        # Read from .gro
        file_name = '/home/michele/BeskowDiag/Select_Q4_Equil/TrjGro/cv'+str(frame).zfill(4)+'.gro'
        x = []
        y = []
        ...
        x_cl = np.take(x, np.where((clustering.labels_==cl_label) * (x<xcom))[0])
        y_cl = np.take(y, np.where((clustering.labels_==cl_label) * (x<xcom))[0])
        ...
        plt.fill_between(y_fun, x_fun, y2=xmax, color='b')
